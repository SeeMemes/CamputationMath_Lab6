...
class ConsoleWorker:
    ...
    def start(self):
        ...
        while 1:
            try:
                self.okay = True
                runge_solver = DiffurSolver(self.type_equation, self.x0, self.y0, self.end_segment, self.step,
                                            self.accuracy)
                runge_solver.startRungeMethod()
                runge_solver_2 = DiffurSolver(self.type_equation, self.x0, self.y0, self.end_segment, self.step / 2,
                                              self.accuracy)
                runge_solver_2.startRungeMethod()
                for i in range(len(runge_solver.y_values)):
                    runge_solver.table_value[i].append(
                        abs((runge_solver.y_values[i] - runge_solver_2.y_values[2 * i]) / 3))
                    if abs((runge_solver.y_values[i] - runge_solver_2.y_values[2 * i]) / 3) > self.accuracy:
                        self.okay = False
                        self.step /= 2
                        break
                if self.okay:
                    print('\nMethod Runge-Kutta:')
                    print_table(runge_solver.table_value)
                    draw_graph(runge_solver.x_values, runge_solver.y_values, runge_solver.type_equation, "Runge-Kutta")
                    del runge_solver, runge_solver_2
                    break
            except IndexError:
                print('Method Runge-Kutta not works\n')
                break

        while 1:
            try:
                self.okay = True
                adams_solver = DiffurSolver(self.type_equation, self.x0, self.y0, self.end_segment, self.step,
                                            self.accuracy)
                adams_solver.startAdamsMethod()
                adams_solver_2 = DiffurSolver(self.type_equation, self.x0, self.y0, self.end_segment, self.step / 2,
                                              self.accuracy)
                adams_solver_2.startAdamsMethod()
                for i in range(len(adams_solver.y_values)):
                    adams_solver.table_value[i].append(
                        abs((adams_solver.y_values[i] - adams_solver_2.y_values[2 * i]) / 15, ))
    ...
    def chooseType(self):
        ...
        while 1:
            ...
            if answer < 1 or answer > 4:
                cprint('Неправильный тип уравнения', 'red')
                continue
            else:
    ...
    def set_start_value(self):
        ...
        while 1:
            ...
            if len(answer) == 2:
                ...
                cprint('Введите правильное количество значений', 'red')
                continue
    ...
    def set_end_segment(self):
        ...
        while 1:
            ...
            if answer > self.x0:
                ...
                cprint('Введите конец сегмента дальше чем его начало')
                continue
    ...
    def chooseStep(self):
        ...
        while 1:
            ...
            if answer > 0:
                ...
                cprint('Введите шаг больше 0', 'red')
                continue
    ...
    def set_accuracy(self):
        ...
        while 1:
            ...
            if accuracy > 0:
                ...
                cprint('Введите точность больше 0', 'red')
                continue
...
class DiffurSolver:
    ...
    def startRungeMethod(self):
        h = self.step
        xspan = np.array([self.x, self.end])
        yinit = np.array([self.y])

        [ts, ys] = self.RungeKutta4thOrder(yinit, xspan, h)
